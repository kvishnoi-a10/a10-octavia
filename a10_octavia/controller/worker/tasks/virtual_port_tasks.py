...
class ListenerUpdateForPool(ListenersParent, task.Task):
    ...
    def execute(self, loadbalancer, listener, pool, vthunder, flow_type=None):
        try:
            if listener:
                kargs = {}
                pool_id = pool[constants.POOL_ID]
...
class ListenerDelete(ListenersParent, task.Task):
    ...
    def execute(self, loadbalancer, listener, vthunder):
        ...
        try:
            self.axapi_client.slb.virtual_server.vport.delete(
                loadbalancer[constants.LOADBALANCER_ID], listener[constants.PROTOCOL], listener[constants.PROTOCOL],
                listener['protocol_port'])
            LOG.debug("Successfully deleted listener: %s", listener[constants.PROTOCOL])
        except (acos_errors.ACOSException, ConnectionError) as e:
            LOG.exception("Failed to delete listener: %s", listener[constants.PROTOCOL])
            raise e
